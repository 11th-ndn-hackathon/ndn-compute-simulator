...
// Atif-Code: Start
  ...
  struct ServiceInfo
  {
    ...
    int node_id;
    int run_time;
    Forwarder::ServiceStatus service_status;
  ...
  virtual Forwarder::ServiceInfo
  UpdateServiceStatus(int service_id, Forwarder::ServiceStatus updated_service_status);
  ...
  virtual vector<Forwarder::ServiceInfo>
  FetchFromServiceCollectionByName(string service_name);

  virtual vector<Forwarder::ServiceInfo>
  FetchFromServiceCollectionByStatus(Forwarder::ServiceStatus service_status);
  ...
  virtual ns3::Ptr<ns3::Node> 
  GetCurrentNode();

  virtual void 
  ProcessContentStoreHit(const FaceEndpoint& ingress, const shared_ptr<pit::Entry>& pitEntry,
